...
confirmed_cases %>% 
  ...
  geom_line(data = SYD_trains, aes(x=lon, y=lat, group = `Railway line(s)`),
             size=.5, col="darkorange") +
  geom_line(data = SYD_ferries, aes(x=lon, y=lat),
             size=.5, col=DC[3]) +
  geom_line(data = SYD_lightrails, aes(x=lon, y=lat),
             size=.5, col="red") +
...
bind_rows(
  SYD_shops %>% select(lon, lat) %>% 
    rename(Long = lon, Lat = lat) %>% mutate(obj = "Supermarket & groceries"), 
  ...
  ggc(fc = c("darkred", "darkblue", "darkgreen")) + 
  theme_void() +
...
confirmed_cases %>% 
  ...
  aes(x=Long, y=Lat, 
      fill=..level.., alpha=..level.., col=obj), 
  binwidth = 1.2, geom="polygon", size=.1) + 
  expand_limits(x = tidy(SYD_POA)$long, y = tidy(SYD_POA)$lat) +
  xlim(150.7,151.48) + ylim(-34.1,-33.5) +
  # scale_fill_gradient(low="white", high=DC[7]) + 
  scale_alpha_continuous(range = c(0,.3)) + 
